...
function makeConnections(data) {
  console.debug('Calculating connections')
    ...
    for (let pointA of relevantPoints) {
      ...
      for (let pointB of relevantPoints) {
        if (pointA.id && pointB.id) {
          ...
          if (JSON.stringify(pointA) !== JSON.stringify(pointB) && pointA.location && pointB.location && !hashes.includes(hash)) {
            hashes.push(hash)
            connections.push({
              hash: hash,
              A: pointA.location,
  ...
  fs.writeFile(
    ...
    'utf8',
    () => {
      console.log('Done calculating connections');
    },
  );
}
